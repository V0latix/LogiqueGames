"""Backtracking solver with forced-move propagation for Zip."""
...
def solve_forced(puzzle: ZipPuzzle, time_limit_s: float | None = None) -> SolveResult:
    timer = Timer()
    timer.start()
    ...
    def dfs(current: Cell) -> bool | None:
        ...
        while True:
            ...
            if not neighbors:
                ...
                for _ in range(len(forced)):
                    state.path.pop()
                state.visited.difference_update(forced)
                metrics.backtracks += 1
                return False
            if len(neighbors) > 1:
                break
